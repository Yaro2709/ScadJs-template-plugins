...
function Plugin_ActivateUI(engine)
{
    try {
        ...
        //получение объекта для работы с файловой системой
        var fso = new ActiveXObject("Scripting.FileSystemObject");
        //Проверка наличия файла
        if (!fso.FileExists(pluginPath + "nodeArr.json"))
        {
            throw("Отсутствуют файл - nodeArr.json")
        }
    
        var path = pluginPath +"nodeArr.json";//путь к файлу
        ///путь к файлу открытие файла
        var jsonFile = fso.OpenTextFile(path, 1);
        var jsonTxt = jsonFile.ReadAll();
        jsonFile.Close();
    
     

        // //Запись json  в параметры SCAD
        engine.SetParameter("nodeArr", jsonTxt, true);
        engine.Execute();
...
function Plugin_Execute(engine) {
    try {
        ...
        //получение параметров из SCAD
        var jsonTxt = engine.GetParameter("nodeArr", []);
        //Десериализация json - перевод в объект, в данном случае в массив с узлами
        var nodeArr = JSON.parse(jsonTxt);

        //Задание узлов из полученного объекта, вытаскиваем из него координаты для создания узлов
        var node_1 = CreateNode(editor, nodeArr[0].x, nodeArr[0].y, nodeArr[0].z);
        var node_2 = CreateNode(editor, nodeArr[1].x, nodeArr[1].y, nodeArr[1].z);
        //создание балки
        var beam = new BeamSCAD(editor, node_1, node_2, 4, 1);
        beam.Create();
