 ...
 * @param {boolean} enableCellPillar включить шпренгель: true - включить
 ...
 function TrussMolodechReversTyp2(
    ...
    function CreateTopBeam() {
        ...
        if (_enableRacksBeam) {
            ...
            topNodeArr = topBeam.GetAllNodes(); //список узлов верхнего пояса
            [].push.apply(topNodeArr, topBeamR.GetAllNodes().reverse()); //добавление узлов верхнего пояса от правой части
            topNodeArr = UniqArrObjNode(topNodeArr);
        }
        //Заполнение номерами
    ...
    */
    ...
    /**
 * Создание балки c 1 расстоянием равным половине основного расстояния(Для фермы с реверсивной сеткой и половиной панели вначале):
 			-с разбиением на части
			-с учетом раного расположения узлов
            -c включением шарниров
 ...
 */
    function BeamOffT2(editor, startNode, endNode, step, joint) {
        ...
        /**
         * Вычесляет длину региона, длины проекций разбиения в зависимости от координат.
         * @param {object} endPoint - Объект первого узла
         * @param {object} startPoint - Объект последнего узла
         * @param {number} step - Кол-во разбиений
         * @returns {object} Объект с проекциеи длин по всем координатам
